;;; generic-dl.el --- Menu for listing toplevel definitions

;; Copyright (C) 2000, 2001, 2002, 2003 by Olaf Sylvester
...
(defvar dl-mode-descriptions
  '(
    ...
    (java-mode :toplevel-matches
	       (("\\(public\\|private\\|protected\\)?\\s +class\\s +\\(\\w+\\)" (" Class %-20s (%s)" 2 1))
		("\\(public\\|private\\|protected\\)\\s +\\(.+\\s \\)?\\(\\w+\\)\\s *\\((.*)\\)\\s *\\({\\|\n\\s *{\\)" ("   Function %-20s (%s)  %s" 3 1 4))
		)
	       :sort-function     (lambda (l1 l2) (string< (car l1) (car l2)))
	       :search-direction  forward)
    ))
(setq dl-mode-descriptions-test
  '(
    (emacs-lisp-mode :toplevel-matches
		     (("^(defun\\s +\\(.*\\)\\s +(" ("  Function %s" 1))
		      ("^(def\\(var\\|custom\\)\\s +\\(.*\\)" ("  Variable %s" 2))
